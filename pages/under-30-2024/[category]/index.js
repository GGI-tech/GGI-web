import profiles from '../profiles-2024.json';
            ...
            <div className='grid md:grid-cols-2 gap-5'>
              <div className="flex items-center  ">
                <div className="h-[300px] sm:h-[500px] w-full">
                  <img src={currentProfile.img} alt="" className="h-full w-full object-contain" />
                </div>
              </div>

              <div>
                <p className='font-bold text-black'>{currentProfile.occupation}| {currentProfile.location}</p>
...
export default function Category({ category }) {
  ...
  function openPopup(profileName) {
    setShowPopup(true);
    router.push(
      `/under-30-2024/${category}?profile=${profileName}`,
      undefined,
      { shallow: true }
    );
    const currentProfile = categoryProfiles.find(
    ...
    {/* flex box  */}
      <div className='mt-36 w-11/12 mx-auto flex justify-center flex-wrap md:flex-nowrap flex-row gap-20'>
        ...
        <div className='md:max-w-1/2'> {/*change here*/}
          <img src={categoryProfiles[0].img} alt="" className="h-full w-full object-contain hover:shadow-lg hover:shadow-[#FF7F50] my-5" />
            ...
            <div key={profile.name} className="hover:cursor-pointer">
              <a onClick={() => openPopup(profile.name)}>
                <div className="h-[300px] md:h-[500px] w-full">
                  <img src={profile.img} alt="" className="h-full w-full object-contain hover:shadow-lg hover:shadow-[#FF7F50] my-5" />
                </div>
                <p className='text-center pt-4 uppercase text-black font-bold'>{profile.occupation}| {profile.location}</p>
